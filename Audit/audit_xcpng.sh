...
function Get-PackagePresence() {
    ...
    if sudo yum list installed "${option}" &>/dev/null; then
        Print-Ok "$desc"
    else
...
if (( major > 9 || (major == 9 && minor >= 1) )); then
    ...
    Print-Warn "R8 - RSA key sized defined (SSH version lower than v9.1)"
fi
...
if sudo yum list installed "pam_krb5" &>/dev/null; then
