...
{
    if ! sudo grep -Fxq "PermitUserEnvironment no" "/etc/ssh/sshd_config"; then
        ...
        sudo sed -i "s/#PermitUserEnvironment no/PermitUserEnvironment no/g" /etc/ssh/sshd_config
    else
