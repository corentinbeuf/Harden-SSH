...
{
    if ! sudo grep -Fxq "PubkeyAuthentication yes" "/etc/ssh/sshd_config"; then
    ...
    if ! sudo grep -Fxq "HostKeyAlgorithms ecdsa-sha2-nistp256,ecdsa-sha2-nistp384,rsa-sha2-512,rsa-sha2-256" "/etc/ssh/sshd_config"; then
    ...
    if ! sudo grep -Fxq "PubkeyAcceptedAlgorithms ecdsa-sha2-nistp256,ecdsa-sha2-nistp384,rsa-sha2-512,rsa-sha2-256" "/etc/ssh/sshd_config"; then
    ...
    if ! sudo grep -Fxq "GSSAPIAuthentication yes" "/etc/ssh/sshd_config"; then
    ...
    if ! sudo grep -Fxq "GSSAPICleanupCredentials yes" "/etc/ssh/sshd_config"; then
    ...
    for package in "${packages_to_install[@]}"; do
        if sudo dpkg -l | sudo awk '{print $2}' | sudo grep -qi "$package"; then
    ...
    if ! sudo grep -Fxq "UsePAM yes" "/etc/ssh/sshd_config"; then
        echo -e "${RED}[Task R17] : Please enable PAM auth in SSHd config.${NC}"
    else
        echo -e "${YELLOW}[Task R17] : PAM auth is already enabled${NC}"
    fi
 
    if ! sudo grep -Fxq "PasswordAuthentication yes" "/etc/ssh/sshd_config"; then
...
function Block-PasswordForHighlyPrivilegedUsers ()
{
    ...
    if ! sudo grep -Fxq "Match User root" "/etc/ssh/sshd_config"; then
...
EOF
    ...
    if ! sudo grep -Fxq "Match Group sudo" "/etc/ssh/sshd_config"; then
...
{
    if ! sudo grep -Fxq "KerberosAuthentication no" "/etc/ssh/sshd_config"; then
    ...
    if sudo grep -q pam_krb5 /etc/pam.d/sshd; then
...
{
    if ! sudo grep -Fxq "PermitEmptyPasswords no" "/etc/ssh/sshd_config"; then
...
{
    if ! sudo grep -Fxq "LoginGraceTime 30" "/etc/ssh/sshd_config"; then
...
{
    if ! sudo grep -Fxq "MaxAuthTries 2" "/etc/ssh/sshd_config"; then
