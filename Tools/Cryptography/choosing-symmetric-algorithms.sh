...
function Setup-SymmetricAlgorithms ()
{
    ...
    if ! sudo grep -Fxq "   Ciphers aes128-ctr,aes192-ctr,aes256-ctr" "/etc/ssh/ssh_config"; then
    ...
    if (( major > 6 || (major == 6 && minor >= 3) )); then
        if ! sudo grep -Fxq "   MACs hmac-sha2-512-etm@openssh.com,hmac-sha2-256-etm@openssh.com" "/etc/ssh/ssh_config"; then
            echo -e "${GREEN}[Task R15] : The encryption algorithm shall either be AES128-CTR, AES192-CTR or AES256-CTR. The integrity mechanism shall rely on HMAC-SHA1, HMAC-SHA256 or HMACSHA512.${NC}"
        ...
        if ! sudo grep -Fxq "   MACs hmac-sha2-512,hmac-sha2-256,hmac-sha1" "/etc/ssh/ssh_config"; then
            echo -e "${GREEN}[Task R15] : The encryption algorithm shall either be AES128-CTR, AES192-CTR or AES256-CTR. The integrity mechanism shall rely on HMAC-SHA1, HMAC-SHA256 or HMACSHA512.${NC}"
    ...
    if ! sudo grep -Fxq "Ciphers aes128-ctr,aes192-ctr,aes256-ctr" "/etc/ssh/sshd_config"; then
        echo -e "${GREEN}[Task R15] : The encryption algorithm shall either be AES128-CTR, AES192-CTR or AES256-CTR. The integrity mechanism shall rely on HMAC-SHA1, HMAC-SHA256 or HMACSHA512.${NC}"
    ...
    if (( major > 6 || (major == 6 && minor >= 3) )); then
        if ! sudo grep -Fxq "MACs hmac-sha2-512-etm@openssh.com,hmac-sha2-256-etm@openssh.com" "/etc/ssh/sshd_config"; then
            echo -e "${GREEN}[Task R15] : The encryption algorithm shall either be AES128-CTR, AES192-CTR or AES256-CTR. The integrity mechanism shall rely on HMAC-SHA1, HMAC-SHA256 or HMACSHA512.${NC}"
        ...
        if ! sudo grep -Fxq "MACs hmac-sha2-512,hmac-sha2-256,hmac-sha1" "/etc/ssh/sshd_config"; then
            echo -e "${GREEN}[Task R15] : The encryption algorithm shall either be AES128-CTR, AES192-CTR or AES256-CTR. The integrity mechanism shall rely on HMAC-SHA1, HMAC-SHA256 or HMACSHA512.${NC}"
