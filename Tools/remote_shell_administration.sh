...
function Get-SSHPresence() {
    ...
    if ! sudo apt list --installed openssh-server &>/dev/null; then
    ...
    if ! sudo systemctl is-active --quiet ssh &>/dev/null; then
        sudo systemctl start sshd
    fi

    if ! sudo systemctl is-enabled --quiet ssh &>/dev/null; then
...
function Remove-OldProtocols() {
    ...
    for package in "${packages_to_remove[@]}"; do
        if sudo dpkg -l | sudo awk '/^ii/ {print $2}' | sudo grep -qw "$package"; then
...
{
    if ! sudo grep -Fxq "PermitTunnel no" "/etc/ssh/sshd_config"; then
    ...
    if ! sudo grep -Fxq "AllowTcpForwarding no" "/etc/ssh/sshd_config"; then
    ...
    if ! sudo grep -Fxq "X11Forwarding no" "/etc/ssh/sshd_config"; then
